...
def create_embeddings_for_csv(
    ...
    for i, row in df.iterrows():
        ...
        point = PointStruct(
            id=str(uuid4()),
            vector=embedding,
            payload={"document_id": doc_id, "row_index": i, "content": text},
        )
        points.append(point)
    return points
...
def upload_embeddings(qdrant_client: QdrantClient, points: Sequence[PointStruct]):
    if not points:
        ...
        return
    qdrant_client.upsert(collection_name=COLLECTION_NAME, points=list(points))

def query_embeddings(
    openai_client: OpenAI, qdrant_client: QdrantClient, query: str, top_k: int = 1
):
